...
public class ManageBooksTest {
	...
	public void testGetBookMap() throws Exception {
		Map<String, Book> bookMap = manageBooks.getBookMap();
		assertEquals(3, bookMap.size());
	}
	
	@Test(expected = Exception.class)
	public void testBorrowReturnBookIssueLimitReached() throws Exception {
		manageBooks.borrowReturnBook(issuedBooks, "test-book2", "borrow");
	}
	
	@Test(expected = Exception.class)
	public void testBorrowReturnBookNotEnoughCopies() throws Exception {
		manageBooks.borrowReturnBook(new ArrayList<>(), "test-book3", "borrow");
	}
	
	@Test
	public void testBorrow() throws Exception {
		Map<String, Book> bookMap = manageBooks.borrowReturnBook(new ArrayList<>(), "test-book2", "borrow");
		assertEquals(1, bookMap.get("test-book2").getAvailableCopies());
	}
	
	@Test
	public void testReturnBook() throws Exception {
		Map<String, Book> bookMap = manageBooks.borrowReturnBook(issuedBooks, "test-book", "return");
