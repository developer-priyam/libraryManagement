...
public class ManageUsersTest {
	...
	public void testGetUser() throws Exception {
		User user = manageUsers.getUser("test-user");
		assertEquals("test-user", user.getName());
	}
	
	@Test(expected=Exception.class)
	public void testGetUserNotFoundException() throws Exception {
		manageUsers.getUser("test-user1");
	}
	
	@Test
	public void testUpdateUserAccountForBorrow() {
		User user = manageUsers.updateUserAccount("test-user", "test-book", "borrow");
		assertEquals(1, user.getIssuedBooks().size());
	}
	
	@Test
	public void testUpdateUserAccountForReturn() {
		User updatedUser = manageUsers.updateUserAccount("test-user2", "test-book", "return");
